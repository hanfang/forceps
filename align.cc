...
int cmp(char s, char t, int allowgu)
{
  if (s == t) { return MATCH; }
  if ((allowgu) && (s == 'T') && (t == 'C')) { return MATCH; }
  return MISMATCH;
}
                  ...
                  int endfree, int allowgu, int V)
{
  ...
  for (int j = 1; j <= m; j++)
  {
    ...
    for (int i = 1; i <= n; i++)
    {
      ...
      int matchscore = scores[i-1][j-1] + cmp(S[i-1], T[j-1], allowgu);
      char tb;
...
/// try change void to int
int global_align_aff(const string & S, const string & T, 
	string & S_aln, string & T_aln,
	int endfree, int allowgu, int V)
{
  S_aln.clear();
  ...
  for (j = 0; j <= m; j++) { X[0][j] = mk_cell(GAP_OPEN +  (j-1)*GAP_EXTEND, '^'); M[0][j] = X[0][j]; }

  if (endfree)
  {
    for (i = 0; i <= n; i++) { Y[i][0] = mk_cell(0, '*'); M[i][0] = Y[i][0]; }
  }
  else
  {
    for (i = 0; i <= n; i++) { Y[i][0] = mk_cell(GAP_OPEN + (i-1)*GAP_EXTEND, '<'); M[i][0] = Y[i][0]; }
  }
  ...
  for (int j = 1; j <= m; j++)
  {
    for (int i = 1; i <= n; i++)
    {
      ...
      M[i][j] = maxscorexy(X[i][j], 
                           Y[i][j],
                           M[i-1][j-1].score+cmp(S[i-1],T[j-1], allowgu));
    }
  }

  // Print the matrix

  if (V) { cout << " "; cout << "     0"; for (int i = 1; i <= n; i++) { cout << "     " << S[i-1]; } cout << endl; }

  for (int j = 0; j <= m; j++)
  {
    if (V) { if (j > 0) { cout << T[j-1]; } else { cout << "0"; } }
